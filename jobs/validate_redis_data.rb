...
class ValidateRedisData
  ...
  class << self
    ...
    def perform
      ...
      passwords = Hash.new
      users.each do |user|
        encrypted_password = rhosync_api.get_db_doc("username:#{user}:password", 'string')
      ...
      users.each do |user|
        ...
        contact_docs = rhosync_api.list_source_docs('Contact', user)      
        contact_data = rhosync_api.get_db_doc( contact_docs['md'] )
        ...
        opp_docs = rhosync_api.list_source_docs('Opportunity', user)      
        opp_data = rhosync_api.get_db_doc( opp_docs['md'] )

...
class DeactivateInactiveUser
  ...
  class << self
    def perform
      ...
      ExceptionUtil.rescue_and_continue do
        ...
        users.each do |user|
          ...
          device_infos =  rhoconnect_api.get_db_doc("source:application:#{user}:DeviceInfo:md")
          devices = rhoconnect_api.get_user_devices(user)
          ...
          if old_devices.nil? || old_devices.count == 0
            ...
            rhoconnect_api.push_deletes('DeviceInfo',user,old_devices) unless old_devices.empty?
            ...
            old_devices.each {|old_device_id| rhoconnect_api.delete_device(user,old_device_id)} unless old_devices.empty?
          end
  
          current_devices = rhoconnect_api.get_user_devices(user)
          if (current_devices.nil? || current_devices.count <=0)
            ...
            rhoconnect_api.delete_user(user)
          end  
        end
      end
    ...
    def disable_user(user)
      begin
        rhoconnect_api.set_user_status(user,'disabled')
      rescue Exception => e
        ExceptionUtil.print_exception(e)
